...
public class DiscodeitApplication {
    ...
    private static void channelView(ConfigurableApplicationContext context) {
        ...
        ChannelService channelService = context.getBean("basicChannelService",
    ...
    private static void messageView(ConfigurableApplicationContext context) {
        ...
        MessageService messageService = context.getBean("JCFMessageService",
            JCFMessageService.class);
        ...
        while (run) {
            ...
            if (selected == MANAGE_MESSAGE.CREATE_MESSAGE) {
                ...
                System.out.println("수정할 메시지 ID: ");
                String messageUUIDString = sc.nextLine();
                UUID messageId = UUID.fromString(messageUUIDString);
    ...
    private static void userView(ConfigurableApplicationContext context) {
        ...
        while (run) {
            ...
            if (selected == MANAGE_USER.CREATE_USER) {
                ...
                userService.create(new CreateUserRequest(name, email, password));
                ...
                userService.update(uuid, new UpdateUserRequest(newName, email, password));
